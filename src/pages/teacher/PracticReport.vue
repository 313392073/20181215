                    ...
                    <table>
                        ...
                        <tbody>
                            <tr v-for="(item,index) in scoreDetail" :key="index" :class="index>4?trHide:''">
                                <td class="td-left" @click="goDetail(item.user_loginname)"><i>{{index+1}}</i><img :src="item.user_head_image" class="head-pic" :alt="item.user_name"><span>{{item.user_name}}</span></td>
...
methods: {
            ...
            series : [
                {
                    ...
                    label: {
                        normal: {
                            show: true,
                            position: 'inside',
                            formatter: "{c}({d}%)",//模板变量有 {a}、{b}、{c}、{d}，分别表示系列名，数据名，数据值，百分比。{d}数据会根据value值计算百分比
                            textStyle : {                   
                                color:'#696767',
                                align:'center',
                                verticalAlign:'middle',
    ...
    goDetail(userloginname) {
        this.$router.push({name:'StudentDetail',params:{userloginname:userloginname,type:0}})
    },
    getInit(params) {
        let self = this;
         base.getUrl(API.allUrl.onlineTest,params).then((res) => {
             console.log(res)
            if(res.code == 200 && res.success == 1) {
                ...
                self.allTestInfo.forEach((item,index) => {
                    let obj = {
                        name:'',
                        value:''
                    };
                    obj.name = '第'+(index*1+1)+'题';
                    obj.value = (item.right_usernum/self.scoreDetail.length).toFixed(2);
                    self.dataArr.push(obj)
                })
            }
        })
    }
